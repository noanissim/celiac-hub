  ...
  return (
    <div className="absolute top-4 right-4 z-[1000] w-80 max-h-[calc(100vh-8rem)] overflow-y-auto rounded-xl border bg-card shadow-xl animate-in slide-in-from-right-5 duration-300">
      <div className="relative p-5">
        <button onClick={onClose} className="absolute top-3 right-3 rounded-full p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground transition-colors">
          <X className="h-4 w-4" />
        </button>
        ...
        {cart.description && <p className="text-sm text-muted-foreground mb-4 border-t pt-3">{cart.description}</p>}
        <div className="flex gap-2">
          {cart.menuLink && (
            <Button size="sm" variant="outline" className="flex-1" asChild>
              <a href={cart.menuLink} target="_blank" rel="noopener noreferrer">
                <ExternalLink className="h-3.5 w-3.5" /> CoffeeTrail
              </a>
            </Button>
          )}
          <Button size="sm" variant="outline" className="flex-1" onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${cart.lat},${cart.lng}`, "_blank")}>
...
export default function CoffeeCartsMap() {
  ...
  const { data: coffeeCarts = [], isLoading } = useCoffeeCarts();
  ...
  return (
    <div className="relative h-[calc(100vh-4rem)] w-full">
      ...
      {selectedCart && <CartDetailPanel cart={selectedCart} onClose={() => setSelectedCart(null)} />}
    </div>
  );
}
